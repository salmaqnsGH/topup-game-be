...
module.exports={
    index: async(req,res)=>{
        try{
            ...
            res.render('admin/bank/view_bank', {bank,alert, name: req.session.user.name, title: "Bank"})
        }catch(err){
    ...
    viewCreate: async(req,res)=>{
        try{
            res.render('admin/bank/create',{name: req.session.user.name, title: "Tambah Bank"})
        }catch(err){
            req.flash('alertMessage', `${err.message}`)
            req.flash('alertStatus', 'danger')
            res.redirect('/bank')
